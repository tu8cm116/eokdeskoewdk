...
CHANNEL_USERNAME = "@RACERSrch"
CHANNEL_URL = "https://t.me/RACERSrch"
...
async def next_cmd(msg: types.Message):
    ...
    # Если был собеседник, запускаем поиск и для него (без уведомления)
    if partner:
    ...
    try:
        if d.startswith("ban_"):
            ...
            if db_pool:
                async with db_pool.acquire() as conn:
                    await conn.execute("UPDATE users SET banned = FALSE WHERE user_id = $1", uid)
            await bot.send_message(uid, "🎉 Поздравляем, вы были разблокированы модерацией. Ваши жалобы обнулены. Впредь, соблюдайте правила. Приятного общения.", reply_markup=main
