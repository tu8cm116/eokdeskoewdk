...
# ---------- КАНАЛ ----------
RACERS_CHANNEL = "@RACERSrch"

# Клавиатура подписки
subscribe_kb = InlineKeyboardMarkup()
subscribe_kb.add(InlineKeyboardButton("Подписаться на @RACERSrch", url="https://t.me/RACERSrch"))
...
async def check_subscription(uid: int) -> bool:
    if uid == MODERATOR_ID:
        return True
    try:
        member = await bot.get_chat_member(chat_id=RACERS_CHANNEL, user_id=uid)
        return member.status in ["member", "administrator", "creator"]
    except Exception as e:
        log.error(f"Ошибка проверки подписки {uid}: {e}")
        return False
...
async def search_for_user(uid):
    ...
    if not await check_subscription(uid):
        await bot.send_message(uid, "Подпишись на @RACERSrch, чтобы искать собеседника:", reply_markup=subscribe_kb)
        return
...
async def start(msg: types.Message):
    ...
    if not await check_subscription(uid):
        await msg.answer(
            "Для использования бота нужна подписка на официальный канал:\n\n"
            "@RACERSrch\n\n"
    ...
    await msg.answer(
        ...
        "Анонимный чат от проекта Racers. Ознакомьтесь с правилами по кнопке «Инфо».\n\n"
        "Выберите действие:",
...
async def check_sub_callback(call: types.CallbackQuery):
    ...
    if await check_subscription(uid):
        await call.message.edit_text("Подписка подтверждена! Теперь доступ открыт.", reply_markup=None)
        await bot.send_message(
            uid,
            "Добро пожаловать в ARMOR.\n\n"
            "Анонимный чат от Racers.\n\n"
            "Выберите действие:",
            reply_markup=main_menu
        )
    else:
...
async def help_cmd(msg: types.Message):
    await msg.answer(
        "С правилами и инструкцией обжалования бана:\n\n"
...
async def report_reason(msg: types.Message):
    ...
    await bot.send_message(partner, "Диалог завершен из-за жалобы.", reply_markup=main_menu)
    if MODERATOR_ID:
        await bot.send_message(
            MODERATOR_ID,
...
async def unban_user(msg: types.Message):
    ...
    await bot.send_message(uid, "Поздравляем, вы разблокированы. Жалобы обнулены. Соблюдайте правила.", reply_markup=main_menu)
        ...
        if d.startswith("ban_"):
            ...
            for r in memory_reports:
                if r['id'] == rid:
                    r['ignored'] = True
                    break
            await call.answer("Жалоба скрыта")
            ...
            if db_pool:
                async with db_pool.acquire() as conn:
                    await conn.execute("UPDATE users SET banned = FALSE WHERE user_id = $1", uid)
            await bot.send_message(uid, "Поздравляем, вы разблокированы. Жалобы обнулены.", reply_markup=main_menu)
            await clear_complaints(uid)
...
async def relay(msg: types.Message):
    ...
    try:
        if msg.text:
            ...
            await bot.send_message(partner, "Тип сообщения не поддерживается.")
    except Exception as e:
        log.error(f"Ошибка: {e}")
